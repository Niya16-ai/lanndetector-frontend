...
function App() {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      const res = await axios.post('https://lanndetector-backend.onrender.com/predict', formData);
  ...
  return (
    <div style={{ padding: '2rem', fontFamily: 'sans-serif', backgroundColor: '#fff8e1' }}>
      <h1 style={{ color: '#b71c1c' }}>อักษรล้านนา Detector</h1>
      <input type="file" multiple onChange={handleUpload} />
      <div style={{ display: 'flex', flexWrap: 'wrap', marginTop: '1rem' }}>
        {results.map((imageResult, idx) => (
          <div key={idx} style={{ margin: '1rem', border: '1px solid #ccc', padding: '1rem' }}>
            <ImageWithBoxes image={images[idx]} detections={imageResult} />
            <div>
              {imageResult.map((det, i) => (
                <div key={i}>
                  <strong>คลาส:</strong> {det.label}<br />
                  <strong>ความมั่นใจ:</strong> {(det.confidence * 100).toFixed(2)}%
                </div>
              ))}
            </div>
          </div>
